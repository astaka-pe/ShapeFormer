...
def convonet_to_shapeformer(array, dim=3, flatten=True):
...
def sparse_convonet_to_shapeformer(pos_ind, shape):
...
def unpack_sparse(sparse, max_length=None, end_tokens=(100, 200)):
    ...
    unique_batch_ind, counts = torch.unique_consecutive(
        batch_ind, return_counts=True)
    ...
    target = end_tokens + \
        torch.zeros(len(counts), counts.max()+1, tuple_n).type_as(sparse)
    # print(target)
    # [pos, val]
    ...
    if max_length is not None and target.shape[1] > max_length:
        ...
        # make sure the last column is end_tokens
        target[:, max_length-1, :] = end_tokens[:, 0, :]
    return target
...
def pack_unpack_unittest():
    sparse = torch.tensor([[0, 4, 1], [0, 5, 2], [1, 1, 5], [
                          2, 3, 2], [2, 5, 1], [3, 0, 0]])
...
def batch_dense2sparse(indices, unpack=True, max_length=None, end_tokens=torch.tensor((100, 200))):
    ...
    packed_sparse = torch.stack((batch_ind, raveled_ind, val), axis=-1)
    if unpack == True:
        unpacked = unpack_sparse(
            packed_sparse, max_length=max_length, end_tokens=end_tokens)
...
def batch_sparse_dense_unittest():
    ...
    ds = batch_sparse2dense(batch_dense2sparse(dense, unpack=False)[
                            0], empty_ind=mode, dense_res=2)
...
def point2voxel(points, grid_dim=32, ret_coords=False):
    ...
    voxel = torch.zeros(
        points.shape[0], *((grid_dim,)*points.shape[-1])).type_as(points)
    ...
    binds = torch.repeat_interleave(torch.arange(
        points.shape[0]).type_as(points).long(), points.shape[1])
    if points.shape[-1] == 2:
    ...
    if ret_coords == True:
        ...
        coords = nputil.makeGrid(
            bb_min=[-1, ]*x_dim, bb_max=[1, ]*x_dim, shape=[grid_dim, ]*x_dim, indexing="ij")
...
def sample_logits(logits, num_samples=1, **filter_kwargs):
    ...
    filtered_logits = [filter_sampling_logits(
        logits[i], **filter_kwargs) for i in range(logits.shape[0])]
    filtered_logits = torch.stack(filtered_logits)
    probs = F.softmax(filtered_logits, dim=-1)
    # print("probs\n",probs.numpy())
    ix_sampled = torch.multinomial(
...
def sample_unittest():
    samples = 10
    logits = torch.tensor(
        [[-1, 0, 1], [1.0001, -2, 1], [0, 0, 1], [1.01, 1, 1.02]])
    sampled = sample_logits(logits, num_samples=samples,
                            top_k=1, top_p=.9, temperature=1.)
    print("top_k=1, top_p=.9\n", sampled.numpy())
    sampled = sample_logits(logits, num_samples=samples,
                            top_k=3, top_p=.9, temperature=1.)
    print("top_k=3, top_p=.1\n", sampled.numpy())
    sampled = sample_logits(logits, num_samples=samples,
                            top_k=3, top_p=.999, temperature=1.)
